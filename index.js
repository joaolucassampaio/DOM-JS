...
function removerJogador() {
    const ul_jgdrsEscalados = document.getElementById('lista-jogadores')
    const li_jgdrsEscalados = ul_jgdrsEscalados.document.getElementByTagName('li')

    const numJgdrRemover = prompt('Qual é o número da camisa do jogador que deseja remover?')

    //Verificar se getElementById armazena em algo tipo uma array, pq explica o .length
    for(let i = 0; i < li_jgdrsEscalados.length; i++){
        const jgdrNumero = li_jgdrsEscalados[i].querySelector('p#num-jogador')

        if (jgdrNumero.innerText === numJgdrRemover) {
            const confirmacao = confirm(
                "Você deseja remover esse jogador?\n" +
                "\nNome do jogador: " + li_jgdrsEscalados[i].querySelector('p#nome-jogador').innerText +
                "\nPosição: " + li_jgdrsEscalados[i].querySelector('p#pos-jogador').innerText +
                "\nNúmero da camisa: " + jgdrNumero.innerText
            )
